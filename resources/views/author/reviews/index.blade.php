                    ...
                    <div class="header">
                        <h2>
                            <a href="{{ route('user.review.index') }}">
                                {{ __('ALL REVIEWS') }}
                                {{-- <span class="btn btn-warning sm">{{ $posts->reviews->count() }}</span> --}}
                            </a>
                        </h2>
                    </div>
                    <div class="body">
                            ...
                            <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                                ...
                                <tbody>
                                    ...
                                    @foreach($posts as $key=>$post)
                                     @foreach($post->reviews as $key=>$review)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            ...
                                            <td >
                                                ...
                                                <form id="delete-form-{{ $review->id }}" action="{{ route('user.review.destroy',$review->id) }}" method="POST" style="display: none;">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </td>
